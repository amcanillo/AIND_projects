...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for act in self.a_levels[level-1]:
            for lit in act.effnodes:
        # 2. connect the nodes
                nodes.add(lit)
                lit.parents.add(act)
                act.children.add(lit)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for sa1 in node_a1.effect_s_nodes():
            for sa2 in node_a2.effect_s_nodes():
                if sa1.__eq__(sa2) & sa1.is_pos == sa2.is_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for sa1 in node_a1.action.effect_add:
            if sa1 in node_a2.action.precond_neg:
                return True

        for sa1 in node_a1.action.precond_neg:
            if sa1 in node_a2.action.effect_add:
                return True

        for sa1 in node_a1.action.effect_rem:
            if sa1 in node_a2.action.precond_pos:
                return True

        for sa1 in node_a1.action.precond_pos:
            if sa1 in node_a2.action.effect_rem:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for sa1 in node_a1.parents:
            for sa2 in node_a2.parents:
                if sa1.is_mutex(sa2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # for s2_p in node_s2.parents:
        #     if s2_p not in node_s1.parents:
        #         return True
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for l in range(len(self.s_levels)):
                for state in self.s_levels[l]:
                    # print("g :", node, "state :", state.symbol)
                    if node == state:
                        found = True
                        # print("found")
                        level_sum += l
